		...
		{ // TODO: remove this extra brace!
			{ Fl_Group* o = m_viewPane = new Special_Tile(250, 25, 550, 600);
				...
				{ curvePane = new Fl_Group(250, 375, 550, 250);
					...
					m_modelerView->resize(250, 25, 550, 600);
					curvePane->hide();
					m_viewPane->init_sizes();
				}
...
void ModelerUserInterface::TabsCallback(Fl_Tabs* tabs, void* p) {

	if ((Fl_Tile*)tabs->value() == ((ModelerUserInterface*)p)->m_controlSplitPane)
	{
		((ModelerUserInterface*)p)->m_modelerView->resize(250, 25, ((ModelerUserInterface*)p)->m_viewPane->w(), ((ModelerUserInterface*)p)->m_viewPane->h());
		((ModelerUserInterface*)p)->curvePane->hide();
		((ModelerUserInterface*)p)->m_viewPane->init_sizes();
	}
	else
	{
		((ModelerUserInterface*)p)->m_modelerView->resize(250, 25, ((ModelerUserInterface*)p)->m_viewPane->w(), ((ModelerUserInterface*)p)->m_viewPane->h() - ((ModelerUserInterface*)p)->curvePane->h());
		((ModelerUserInterface*)p)->curvePane->show();
		((ModelerUserInterface*)p)->m_viewPane->init_sizes();
	}
}
...
void ModelerUserInterface::setMovieLength() {
	...
	char lastLength[10];
	float testLength = 1;
	sprintf(lastLength, "%d", (int)graph->endTime());
	do {
...
ParticleSystem* ModelerUserInterface::getParticleSystem() { 
	return ps; 
}
